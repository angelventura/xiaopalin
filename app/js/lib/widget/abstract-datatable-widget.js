...
console.log("Loading file:abstract-datatable-widget.js ...");/*global document:false, sessionStorage: false, console: false, alert: false, $: false, window: false, jQuery:false,  location:false, debugger:false, navigator:false, localStorage:false,google: false */
...
const SINGLETON=class AbstractTableWidget extends AbstractWidget {
        ...
        if (this.$datatable === null) {
            ...
            return this.$datatable.on(eventType,selector, (event) => {
                try {
                    ...
                    Log.error("File:abstract-datatable-widget, Error on event type:"+eventType,err);
                }
            });
        }
    }
    ...
    enableRowsChild(f,v,selector){
        ...
        if (!f){
            alert("abstract-datatable-widget: No enableRowsChild f defined");
        }
        if (!selector){
            selector="tr";
        }
        ...
        this.$datatable.on('click',selector,(event) =>{
            try {
                ...
                Log.error("File:abstract-datatable-widget, Error on click",err);
            }
        });        
    }
    ...
    display($el){
        ...
        if ($el) {        
            ...
            this.$el=El.assert(`#${this.id}-table`,"abstract-datatable-widget",this.id);
        }
