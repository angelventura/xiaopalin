...
console.log("Loading file:abstract-url-search-widget.js ...");/*global document:false, sessionStorage: false, console: false, alert: false, $: false, window: false, jQuery:false,  location:false, debugger:false, navigator:false, localStorage:false,google: false */
...
const SINGLETON=class InputUrl extends Parent {
    ...
    constructor(paramName,defaultValue,selector,idField){
        super("abstract-url-search-widget",paramName,defaultValue);
    ...
    _buildUrlSelector(){
        ...
        if (!$selector){
            alert("File: abstract-url-search-widget, id:"+this.id+", selector not found: "+this.selector);
            Log.error("File: abstract-url-search-widget, id:"+this.id+", selector not found: "+this.selector);
            return ;
        } else {
            const that=this;
            ...
            $q(promise)
                ...
                .catch( (err) =>{
                    Log.error("File: abstract-url-search-widget, error",err);
                    $selector.removeClass("loading");
                    $selector.addClass("disabled");                    
                });
